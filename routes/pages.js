...
router.get('/employees', (req, res)=>{
    res.render('Employees', {msg: req.query.msg});
});
...
router.post('/save-employee', (req, res)=>{
    console.log(req.body);
        if(req.body.name === '' && req.body.name.length <= 2 && req.body.name.length >= 20) {
            ...
            res.redirect('/employees?msg=Oops, employmentDate cannot be blank');
        } else if(req.body.terminationDate === '' ){
            res.redirect('/employees?msg=Oops, confirm terminationDate cannot be blank');
        }
        else {
